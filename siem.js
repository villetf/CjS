...
if (document.getElementsByClassName('heading')[0]) {
   ...
   ticketImage.src = 'https://siem.domain.se/js/themes/shim.gif';
   ...
   linkImage.src = 'https://siem.domain.se/js/themes/shim.gif';
   ...
   autoTicketImage.src = 'https://siem.domain.se/js/themes/shim.gif';
...
function createTicket() {
   copyOffenseLink(eventNumber => {
      window.open('https://siem.domain.se/console/properties?offense&id=' + eventNumber);
      window.open('https://servicedesk.domain.se/WorkOrder?woMode=newWO&reqTemplate=5453');
   });
}
...
function copyOffenseLink(callback) {
   ...
   navigator.clipboard.writeText('https://siem.domain.se/console/siem?summaryId=' + eventNumber)
      .then(() => {
...
async function autoGenerateTicket() {
   ...
   const title = (document.getElementsByClassName('heading')[0]).innerText;
   const eventNumber = title.replace('Offense ', '');
   setCookie('siemEventnumber', eventNumber, 5);
   setCookie('siemSubject', `SIEM - ${rows[elementIndex].textContent}`, 5);
   setCookie('siemDescription', encodeURIComponent(`<div>${rows[elementIndex].textContent}<br /></div><div><br /></div><div>Offense: ${eventNumber}<br /></div><div><br /></div><div style="text-align: left" dir="ltr"><a href="https://siem.domain.se/console/siem?summaryId=${eventNumber}" target="_blank">https://siem.domain.se/console/siem?summaryId=${eventNumber}</a><br /></div>`), 5);
   ...
   const findCookieInterval = setInterval(async() => {
      ...
      const servicedeskTicketNumber = getCookie('servicedeskTicketNumber');
      const headers = {
         'Version': '20.0',
         'Accept': 'application/json',
         'SIEMcsrf': (getCookie('siemCSRF'))
      };
      const commentText = `https://servicedesk.domain.se/WorkOrder?woMode=viewWO&woID=${servicedeskTicketNumber} //${getCookie('servicedeskUsername')}`;
      const siemCommentCall = await fetch(`https://siem.domain.se/api/siem/offenses/${eventNumber}/notes?note_text=${encodeURIComponent(commentText)}`, {
         method: 'POST',
         headers: headers
      });

      if (!siemCommentCall.ok) {
         alert('Lyckades inte skapa SIEM-kommentar.');
      }
      sendLog(29, `#${servicedeskTicketNumber}`);
