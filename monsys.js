...
// Skriptet förvaras på serverx. Länken till skriptet är https://serverx.domain.se/cjs/monsys.js.
//
...
if (getCookie('logsDisabled')) {
   ...
   if (window === window.top) {
      ...
      if (document.getElementById('filter-query-multi-action')) {
         // eslint-disable-next-line no-restricted-syntax -- Style-egenskapen är nödvändig eftersom det är ett värde som Övervakningssystem redan använder som vi vill ändra.
         document.getElementById('filter-query-multi-action').style.top = '135px';
      }
   }
}

// Kod för att lägga till knappar till checksidan i Övervakningssystem
if (window.location.href.startsWith('https://monsys.domain.se/monitor/index/') || window.location.href.startsWith('https://monsystest.domain.se/monitor/index/')) {
   addServicepageButtons();
}

// Checka alla-knapp
if (window.location.href.startsWith('https://monsys.domain.se/monitor/index/listview') || window.location.href.startsWith('https://monsystest.domain.se/monitor/index/listview')) {
...
if (window.location.href === 'https://monsys.domain.se/monitor/index/cmd/obj' || window.location.href === 'https://monsystest.domain.se/monitor/index/cmd/obj') {
...
if (window.location.href.startsWith('https://monsys.domain.se/monitor/index/listview') || window.location.href.startsWith('https://monsystest.domain.se/monitor/index/listview')) {
   ...
   if (choiceList.querySelector('ul')) {
      ...
      choiceButton.onclick = () => {
         if (getCookie('logsDisabled')) {
            document.cookie = 'logsDisabled=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; domain=.domain.se';
...
if (window.location.href === ('https://monsys.dommonitor/monsys/dacota/export') || window.location.href === ('https://monsystest.domain.se/monitor/monsys/dacota/export')) {
   const open = document.getElementsByClassName('changelisting')[0];
   // eslint-disable-next-line no-restricted-syntax -- Style-egenskapen är nödvändig eftersom Övervakningssystem använder den för att avgöra vad som ska vara utfällt.
   open.style = 'nothing';
...
function addServicepageButtons() {
   ...
   // Visa i Visualiseringssystem-knapp
   ...
   createGrafButton.innerHTML = 'VISA I VISUALISERINGSSYSTEM';
   createGrafButton.onclick = () => {
      showVisualiseringssystem();
   };
   element.appendChild(createGrafButton);


   // Visa i Nätverkshanteringssystem-knapp
   const NetworkManagementSystemButton = document.createElement('button');
   NetworkManagementSystemButton.id = 'NetworkManagementSystemButton';
   NetworkManagementSystemButton.className = 'servicepage-buttons';
   NetworkManagementSystemButton.innerHTML = 'VISA I Nätverkshanteringssystem';
   NetworkManagementSystemButton.onclick = () => {
      showNetworkManagementSystem();
   };
   element.appendChild(NetworkManagementSystemButton);
   ...
   case 'UP':
      ...
      NetworkManagementSystemButton.classList.add('servicepage-buttons-ok');
      break;
   case 'CRITICAL':
   case 'DOWN':
      ...
      NetworkManagementSystemButton.classList.add('servicepage-buttons-crit');
      break;
   case 'WARNING':
   case 'UNREACHABLE':
      ...
      NetworkManagementSystemButton.classList.add('servicepage-buttons-warn');
      break;
   case 'UNKNOWN':
      ...
      NetworkManagementSystemButton.classList.add('servicepage-buttons-unknown');
      break;
   case 'PENDING':
      ...
      NetworkManagementSystemButton.classList.add('servicepage-buttons-pending');
      break;
   }
}
...
function searchCI() {
   ...
   window.open('https://servicedesk.domain.se/CMDBAction?mode=listView', '_blank');
   sendLog(1, hostname);
}
...
function createRequestAck() {
   ...
   setCookie('Övervakningssystemlink', eventInfo.location, 25);
   setCookie('Övervakningssystemhostname', eventInfo.hostname, 25);
   setCookie('Övervakningssystemoutput', eventInfo.output, 25);
   setCookie('ÖvervakningssystemlastChange', eventInfo.lastChange, 25);
   window.open('https://servicedesk.domain.se/WorkOrder?woMode=newWO&reqTemplate=3523');
...
function autogenerateRequest() {
   setCookie('monsysAutofill', 'true', 5);
   const eventInfo = getEventInfo();
   setCookie('monsysLocation', eventInfo.location, 10);
   setCookie('monsysEventTitle', eventInfo.eventTitle, 10);
   setCookie('monsysHostname', eventInfo.hostname, 10);
   setCookie('monsysoutput', eventInfo.output, 10);
   setCookie('monsysLastChange', eventInfo.lastChange, 10);
   window.open('https://servicedesk.domain.se', '_blank');

   const ticketNumInterval = setInterval(() => {
      if (!getCookie('servicedeskTicketNumber')) {
         return;
      }
      clearInterval(ticketNumInterval);

      const ackDetails = {
         // eslint-disable-next-line camelcase -- Disablas eftersom egenskapen är Övervakningssystem-specifik.
         ...
         notify: true,
         persistent: true,
         comment: `https://servicedesk.domain.se/WorkOrder?woMode=viewWO&woID=${getCookie('servicedeskTicketNumber')} //${getCookie('servicedeskUsername')}`
      };

      fetch('https://monsys.domain.se/api/command/ACKNOWLEDGE_PROBLEM', {
         method: 'POST',
         headers: {
...
// Funktion för att öppna i Visualiseringssystem
function showVisualiseringssystem() {
   const hostname = getParameterByName('host');
   const hostRegex = /.*-.*-.*/g;
   sendLog(3, hostname);
   if (hostRegex.test(hostname)) {
      window.open('https://servery.domain.se/visualsystem/d/tLcwDxnVz/health-switchar?orgId=1&var-host=' + hostname + '&kiosk');
      return;
   }
   window.open('https://servery.domain.se/visualsystem/d/JQ6p8uM4k/health?orgId=1&var-host=' + hostname);
}

// Funktion för att öppna i Nätverkshanteringssystem
function showNetworkManagementSystem() {
   const hostname = getParameterByName('host');
   window.open('https://NetworkManagementSystem.domain.se/search/?q=' + hostname);
   sendLog(4, hostname);
}
...
function showChangelog() {
   const hostname = getParameterByName('host');
   if (window.location.href.startsWith('https://monsystest.domain.se')) {
      window.open('https://monsystest.domain.se/monitor/index/configuration/configure?page=changelog_history%3Fmatch_object%3D' + hostname + '%26match_time_start%3Dmatch_time_stop', '_self');
   } else {
      window.open('https://monsys.domain.se/monitor/index/configuration/configure?page=changelog_history%3Fmatch_object%3D' + hostname + '%26match_time_stop', '_self');
   }
   sendLog(5, hostname);
}
...
function addFireworks(blueElement) {
   ...
   if (!document.getElementById('fireworksElement')) {
      ...
      if (monthIndex === 11) {
         imageelement.src = 'https://serverx.domain.se/cjs/resources/santa-dance.gif';
      } else {
         imageelement.src = 'https://serverx.domain.se/cjs/resources/fireworks.gif';
      }
...
function addListviewButtons() {
   ...
   for (let i = 0; i < filterRows.length; i++) {
      ...
      let buttonRow;
      if (window.location.href.startsWith('https://monsys.domain.se/monitor/index/listview?q=%5Bhosts') || window.location.href.startsWith('https://monsys.domain.se/monitor/index/listview?q=%5Bhosts')) {
         if (!(tempRow.querySelector('td a[href^=\'/monitor/index/configuration/configure/?type=\']'))) {
            continue;
         }
         buttonRow = (tempRow.querySelector('td a[href^=\'/monitor/index/configuration/configure/?type=\']')).parentElement;
      } else if (tempRow.querySelector('td a[href^=\'/monitor/index/configuration/configure/?type=service\']')) {
         buttonRow = (tempRow.querySelector('td a[href^=\'/monitor/index/configuration/configure/?type=service\']')).parentElement;
      }

      // Lägg till kopiera larminfo-knapp i listvyn
      const copyImg = document.createElement('img');
      copyImg.classList.add('customButtons', 'customCopyButton');
      copyImg.src = 'https://serverx.domain.se/cjs/resources/copyicon.png';
      copyImg.title = 'Kopiera larminfo';
      copyImg.onclick = function() {
         copyAlertInfo(tempRow);
         copyImg.src = 'https://serverx.domain.se/cjs/resources/checkicon.png';
         copyImg.title = 'Länk kopierad';
      };

      const monsysRegex = /https:\/\/monsys\.users\.se\/monitor\/index\.php\/listview.*services.*/;
      if (buttonRow && monsysRegex.test(window.location.href) && (!buttonRow.getElementsByClassName('customCopyButton')[0])) {
      ...
      if (monsysRegex.test(window.location.href) && (!buttonRow.getElementsByClassName('icon-16 x16-edit')[0])) {
...
function searchCIFromList(tempRow) {
   ...
   window.open('https://servicedesk.domain.se/CMDBAction?mode=listView&allCIs=true', '_blank');
...
function createReqFromList(tempRow) {
   const alertInfo = getListviewServiceInfo(tempRow);
   setCookie('Övervakningssystemlink', alertInfo.copyLink, 25);
   setCookie('Övervakningssystemhostname', alertInfo.buttonHostname, 25);
   setCookie('Övervakningssystemoutput', alertInfo.buttonOutput, 25);
   setCookie('ÖvervakningssystemlastChange', alertInfo.buttonLastChange, 25);
   window.open('https://servicedesk.domain.se/WorkOrder?woMode=newWO&reqTemplate=3452');
...
function getListviewServiceInfo(tempRow) {
   ...
   if (window.location.href.startsWith('https://monsys.domain.se/monitor/index/listview?q=%5Bservices') || window.location.href.startsWith('https://monsys.domain.se/monitor/index/listview/?q=%5Bservices')) {
      if (!tempRow.querySelectorAll('td[data-table=\'services\']')[0]) {
         return;
      }
      buttonCheckName = (tempRow.querySelectorAll('td[data-table=\'services\']')[0]).getAttribute('data-object');
